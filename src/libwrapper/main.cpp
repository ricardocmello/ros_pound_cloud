...
int main(int argc, char * argv[]){
    ...
    argo_setCommentId(argo_addInt(&num_of_nodes, STR("num-of-nodes"), 2, 1),
                     STR("Specify number of nodes"));
    ...
    // Found a workaround for hardcoding the PATH by using the __FILE__ macro 
    // and removing the substring main.cpp from it
    // check https://thispointer.com/how-to-remove-substrings-from-a-string-in-c/
    const std::string toErase = "main.cpp";
    std::string mainStr = __FILE__;
    size_t pos = mainStr.find(toErase);
 	//if (pos != std::string::npos) {
	mainStr.erase(pos, toErase.length()); //}
    oss1 << mainStr << "config/config.yaml";
    //std::cout << "FILE CONFIG: " << oss1.str();
    //oss1 << getenv("HOME") << "/catkin_ws/src" << "/libwrapper/config/config.yaml";
