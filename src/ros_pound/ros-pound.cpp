 ...
 *-----------------------------------------------------------------------*/
/*  -----------------------------------------------------------------------
 *  Several modifications were made to enable the robot-cloud communication
 *  in PoundCloud, a fork of the original Pound.
 *
 *  Contact Address: Ricardo Mello                   ricardo.mello@ieee.org
 *
 *-----------------------------------------------------------------------*/
...
// map<int, string> node_ip;
/*
 Instead of relying on a sequence of IPs starting at the base IP, 
 we now read from an object mapping node-id to IP.
 Same for port.

 This functionality was not tested with more than two nodes.
*/
YAML::Node node_ip = YAML::Load("{0: 192.168.0.1, 1:127.0.0.1}");
YAML::Node node_port_rx = YAML::Load("{0: 32000, 1: 32000}");
YAML::Node node_port_tx = YAML::Load("{0: 32001, 1: 32001}");


char device[20]; // This will cause buffer overflow if lenght of interface_name > 19. TODO: fix it
//char device[6]="wlan0"; // This will cause buffer overflow if lenght of interface_name > 5
...
void wrapper_setup_config(YAML::Node yaml, int _node_id, int _num_of_nodes, int _num_of_ports){
    ...
    //if (use_ip){
        ...
        if (YAML::Node parameter = yaml["node_ip"]) {
            node_ip = parameter;
        }
        ...
        if (YAML::Node parameter = yaml["node_port_rx"]) {
            node_port_rx = parameter;
        }
        if (YAML::Node parameter = yaml["node_port_tx"]) {
            node_port_tx = parameter;
        }
    // we're removing the use_ip thing
    /*} else {
...
int wrapper_init(ros::NodeHandle & n, int _node_id, int _num_of_nodes, int _num_of_ports){
    ...
    std::cout << "<<<<<<< [DEBUG] RX node_ip: "<< node_ip[node_id].as<std::string>().c_str() << std::endl;
    std::cout << "<<<<<<< [DEBUG] RX node_port: "<< node_port_rx[node_id].as<int>() << std::endl;
    
    // Bind RX socket
    // int address = htonl(ntohl(inet_addr(base_ip)) + _node_id);
    int address_rx = htonl(ntohl(inet_addr(node_ip[node_id].as<std::string>().c_str())));
    rxservaddr.sin_addr.s_addr = address_rx; // htonl(INADDR_ANY);
    // rxservaddr.sin_port = htons(port); 
    rxservaddr.sin_port = htons(node_port_rx[node_id].as<int>()); 
    int res = bind(rxsockfd, (struct sockaddr *) &rxservaddr, sizeof(rxservaddr));
    if (res != 0) {
        fprintf(stderr, "*** ABORTING *** Bind error: do IP address_rx and node-id are coherent in this machine?\n");
        exit(0);
    }
    
    std::cout << "<<<<<<< [DEBUG] TX node_ip: "<< node_ip[node_id].as<std::string>().c_str() << std::endl;
    std::cout << "<<<<<<< [DEBUG] TX node_port: "<< node_port_tx[node_id].as<int>() << std::endl;

    // Bind TX socket
    int address_tx = htonl(ntohl(inet_addr(node_ip[node_id].as<std::string>().c_str())));
    txservaddr.sin_addr.s_addr = address_tx; // htonl(INADDR_ANY);
    txservaddr.sin_port = htons(node_port_tx[node_id].as<int>()); 
    int tx = bind(txsockfd, (struct sockaddr *) &txservaddr, sizeof(txservaddr));
    if (tx != 0) {
        fprintf(stderr, "*** ABORTING *** Bind error: do IP address_tx and node-id are coherent in this machine?\n");
        exit(0);
    }
    ...
    while (1){
        ...
        if (max_priority_id >= 0){
            ...
            if (1){
                ...
                txservaddr.sin_port = htons(node_port_rx[next_hop].as<int>());
